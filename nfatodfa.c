...
void main()
{
 ...
 while(i!=complete)
 {
  ...
  for(k=0;k<noalpha;k++)
  {
   c=0;
   for(j=1;j<=nostate;j++)
   set[j]=0;        //TEMPORARY ARRAY TO STORE NFA states reachable from newstate on this symbol 
   for(j=1;j<=nostate;j++)
   {
    l=newstate.nst[j];
    if(l!=0)
...
void insert(int r,char c,int s)
{
  ...
  exit(0);
       ...
       temp=(struct node *) malloc(sizeof(struct node));//create memry dyanamically each time we are including transitions.
       temp->st=s;
